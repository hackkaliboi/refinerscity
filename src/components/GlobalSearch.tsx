...
export function GlobalSearch() {
  ...
  return (
    <div className="relative w-full max-w-lg mx-auto">
      ...
      <ul
        {...getMenuProps()}
        className={`absolute z-10 w-full bg-white mt-1 rounded-lg shadow-lg border border-primary-200 ${isOpen && (items.length > 0 || isLoading) ? '' : 'hidden'
          }`}
      >
        {isOpen &&
          items.map((item, index) => (
            <li
              ...
              className={`px-4 py-3 cursor-pointer ${highlightedIndex === index
                  ? 'bg-primary-100'
                  : 'hover:bg-primary-50'
            ...
            >
              <a href={item.url} className="block">
                <div className="flex justify-between">
                  <span className="font-medium text-primary-800">{item.title}</span>
        ...
        {isLoading && (
          <li className="px-4 py-3 text-primary-600 flex items-center">
        ...
        {isOpen && !isLoading && items.length === 0 && (
          <li className="px-4 py-3 text-primary-600">
            No results found
          </li>
        )}
      </ul>
